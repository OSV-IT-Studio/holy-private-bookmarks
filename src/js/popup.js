...
function createBookmarkElement(item, path) {
    ...
    spacer.dataset.path = path.join(',');
    spacer.dataset.position = 'after';
...
function showFolderDropIndicator(folder) {
    const oldIndicator = document.querySelector('.folder-drop-indicator');
    if (oldIndicator) oldIndicator.remove();
    
    const indicator = document.createElement('div');
    indicator.className = 'folder-drop-indicator';
    indicator.textContent = '↘️ Drop inside folder';
    
    const subitems = folder.querySelector('.subitems');
    if (subitems) {
        subitems.prepend(indicator);
    }
}

function showPositionIndicator(target, isBefore) {
    const oldIndicator = document.querySelector('.drop-position-indicator');
    if (oldIndicator) oldIndicator.remove();
    ...
    indicator.style.top = (isBefore ? rect.top - 2 : rect.bottom - 2) + 'px';
    indicator.style.left = '30px';
    indicator.style.width = (treeRect.width - 50) + 'px';
    
    document.getElementById('tree').appendChild(indicator);
}
...
function handleDrop(e) {
    ...
    const draggedItemCopy = dragState.draggedItem;
