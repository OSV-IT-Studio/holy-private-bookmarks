...
const HTMLImporter = (function() {
  ...
  function getLocalizedMessage(key, substitutions = []) {
    ...
    if (typeof window !== 'undefined' && window.HolyShared && window.HolyShared.getMessage) {
      return window.HolyShared.getMessage(key, substitutions);
    }
    

    throw new Error('Localization system (HolyShared.getMessage) is not available');
  }
  ...
  async function importFromHTML(file) {
    ...
    try {
      ...
      if (_callbacks.onSuccess) _callbacks.onSuccess(result);

    } catch (error) {
      console.error('HTML import error:', error);
      if (_callbacks.onError) {
        const message = getLocalizedMessage(_messages.parseError, [error.message]);
        _callbacks.onError(message);
      }
    }
  }
