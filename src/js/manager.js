...
function countAllBookmarks() {
  ...
  if (folderCountsCache.has('all') && folderCountsCache.get('all').version === version) {
    return folderCountsCache.get('all').count;
  }
  ...
  folderCountsCache.set('all', { count, version });
  return count;
}
...
function clearBookmarksCache() {
  ...
  console.log('Cache cleared, new version:', data?._version);
}
...
async function renderBookmarks() {
  ...
  if (bookmarks.length === 0) {
    ...
    if (emptyState) {
      ...
      // Показываем разные сообщения в зависимости от того, есть ли поиск
...
function showLoadingIndicator() {
  ...
  if (!indicator) {
    ...
    bookmarksGrid.appendChild(indicator);
  }
}
...
async function moveSelectedBookmarks(targetPath) {
  const bookmarksToMove = Array.from(selectedBookmarks);
  ...
  if (targetPath.length === 0) {
    ...
    if (!folder || folder.type !== 'folder') {
      showNotification('Invalid destination folder', true);
      return;
    }
    targetArray = folder.children;
  }
  ...
  clearSelection();
  showNotification(getMessage('bookmarksMoved') || `${bookmarksToMove.length} bookmarks moved`);
  renderFolderTree();
  renderBookmarks();
}
